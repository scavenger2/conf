...
if [ ! -d "$REPO_PATH/.git" ]; then
    echo "錯誤: $REPO_PATH 不是一個有效的 Git 儲存庫。"
    exit 1
fi
...
read "?請輸入新的 feature 分支名稱 (例如: FOR-123-bugfix): " FEATURE_BRANCH_NAME
if [ -z "$FEATURE_BRANCH_NAME" ]; then
    echo "錯誤: 分支名稱不能為空。"
    exit 1
fi

# 依據命名規則，生成 deploy 分支名稱
if [ -z "$DEPLOY_BRANCH_SUFFIX" ]; then
    DEPLOY_BRANCH_NAME="deploy-${FEATURE_BRANCH_NAME}"
else
    DEPLOY_BRANCH_NAME="deploy-${FEATURE_BRANCH_NAME}${DEPLOY_BRANCH_SUFFIX}"
fi
...
FEATURE_WORKTREE_PATH="$WORKTREES_DIR/$FEATURE_BRANCH_NAME"
...
GIT_HOOKS_DIR="$REPO_PATH/githooks" # 假設 hooks 存放於主儲存庫中

# 為 feature worktree 建立 hooks (只需要 post-commit)
cd "$FEATURE_WORKTREE_PATH"
mkdir -p .git/hooks
ln -s "$GIT_HOOKS_DIR/post-commit" .git/hooks/post-commit
echo "✅ Feature worktree 的 hooks 連結已建立。"

# 為 deploy worktree 建立 hooks (只需要 pre-push)
cd "$DEPLOY_WORKTREE_PATH"
mkdir -p .git/hooks
ln -s "$GIT_HOOKS_DIR/pre-push" .git/hooks/pre-push
